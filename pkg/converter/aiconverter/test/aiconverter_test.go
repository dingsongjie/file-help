...
func TestToFastJpeg(t *testing.T) {
	...
	t.Run("Ai-4 to jpeg", func(t *testing.T) {
		aiAbsolutePath := path.Join(mydir, "./assets/4.ai")
		outputAbsolutePath := path.Join(mydir, "./outputs/ai-4.jpeg")
		converter.ToFastImage(aiAbsolutePath, outputAbsolutePath)
	})
}
func TestToPrettyPdf(t *testing.T) {
	...
	t.Run("Ai-4 to pdf", func(t *testing.T) {
		aiAbsolutePath := path.Join(mydir, "./assets/4.ai")
		outputAbsolutePath := path.Join(mydir, "./outputs/ai-4.pdf")
		converter.ToPrettyPdf(aiAbsolutePath, outputAbsolutePath)
	})
}
